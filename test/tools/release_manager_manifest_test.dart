...
void main() {
  ...
  group('upsertRelease', () {
    ...
    test('replaces an existing entry with the same version', () {
      final m = emptyManifest();
      upsertRelease(m, _release('0.10.4', changelog: 'first'), maxReleases: 10);
      upsertRelease(m, _release('0.10.4', changelog: 'second'), maxReleases: 10);
      final releases = m['releases'] as List;
      expect(releases, hasLength(1));
    ...
    test('keeps newest first across multiple inserts', () {
      ...
      upsertRelease(m, _release('0.10.4'), maxReleases: 10);
      final versions = (m['releases'] as List).map((r) => r['version']).toList();
    ...
    test('preserves arbitrary unknown fields on a release', () {
      ...
      upsertRelease(m, _release('0.10.4', changelog: 'updated'), maxReleases: 10);
      final entry = (m['releases'] as List).first as Map;
      expect(entry['futureField'], isNull, reason: 'replacement is full overwrite for the matching version');
